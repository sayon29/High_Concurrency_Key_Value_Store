...
private:
    ...
    int num_threads;
    int poll_timeout_ms;
    const std::vector<SmallString>& keys;

    void set_nonblocking(int fd) {
        fcntl(fd, F_SETFL, fcntl(fd, F_GETFL, 0) | O_NONBLOCK);
    }

    void bucket_worker(int tid, int conns_to_manage, Metrics& m) {
        std::vector<ConnState> states;
        std::vector<struct pollfd> fds;

        int step = (tid % 4) + 1;
        std::mt19937 local_rng(std::random_device{}() + tid);
            ...
            for (size_t i = 0; i < fds.size(); ++i) {
                ...
                if (fds[i].revents & POLLOUT) {
                    ...
                    if (states[i].status == ConnStatus::SENDING) {
                        ...
                        // Stack buffer: No heap allocation, ultra-fast memory
                        char req_buf[64]; 
                        int req_len = 0;
                        bool is_quit = false;
                        ...
                        if (states[i].reqs_done < reqs_per_conn - 1) {
                            ...
                            // 1. Fetch 4-byte key directly from L1 Cache
                            const SmallString& current_key = keys[states[i].key_idx];
                            
                            // 2. Advance index using thread-specific step
                            states[i].key_idx = (states[i].key_idx + step) % keys.size();

                            // 3. Format directly into stack buffer using C-strings
                            if (states[i].reqs_done % 2 == 0) {
                                req_len = snprintf(req_buf, sizeof(req_buf), "SET %s v\n", current_key.s);
                            } else {
                                req_len = snprintf(req_buf, sizeof(req_buf), "GET %s\n", current_key.s);
                            }

                        } else {
                            req_len = snprintf(req_buf, sizeof(req_buf), "QUIT\n");
                            is_quit = true;
                        }

                        states[i].last_start = high_resolution_clock::now();
                        ssize_t sent = send(fds[i].fd, req_buf, req_len, 0);
                    ...
                    if (r > 0) {
                        ...
                        m.total_requests++;
                        states[i].reqs_done++;
                        states[i].conn_start = high_resolution_clock::now(); 
...
public:
    BenchmarkClient(std::string ip, int port, int threads, int conns_per_t, int reqs_per_conn, int poll_timeout_ms, const std::vector<SmallString>& keys)
        : ip(ip), port(port), num_threads(threads),
          total_conns(conns_per_t * threads), reqs_per_conn(reqs_per_conn),
          poll_timeout_ms(poll_timeout_ms), keys(keys) {}
    ...
    void run() {
        ...
        for (int i = 0; i < num_threads; i++) {
            workers.emplace_back(&BenchmarkClient::bucket_worker, this, i, conns_per_thread, std::ref(all_metrics[i]));
        }
...
int main(int argc, char* argv[]) {
    ...
    std::vector<SmallString> precomputed_keys;
    precomputed_keys.reserve(700);
    ...
    for (int i = 0; i < 700; i++) {
        SmallString key;
        for (int j = 0; j < 3; j++) {
            key.s[j] = charset[main_rng() % (sizeof(charset) - 1)]; 
        }
        key.s[3] = '\0';
        precomputed_keys.push_back(key);
    }
    
    BenchmarkClient bench("127.0.0.1", 8080, concurrency, conns_per_thread, reqs_per_conn, poll_timeout_ms, precomputed_keys);
    bench.run();
    return 0;
}